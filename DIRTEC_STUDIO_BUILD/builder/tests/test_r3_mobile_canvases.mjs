...
assert.equal(state.document.canvases.length, 2);

const clone = state.duplicateNode(first.id);
assert.equal(state.document.canvases.length, 3);
assert.equal(state.getChildren(clone.id).length, 1);
assert.notEqual(state.getChildren(clone.id)[0].id, child.id);
assert.equal(state.getChildren(clone.id)[0].canvasId, clone.id);

state.reorderNode(second.id, 0);
assert.equal(
    [...state.document.canvases].sort((a, b) => a.order - b.order)[0].id,
    second.id,
);
...
assert.equal(state.document.canvases.length, 2);
assert.equal(
    state.document.nodes.some((node) => node.canvasId === clone.id),
    false,
);
