...
export class CanvasManager {
    ...
    createCanvas() {
        const count = this.state.document.sections.length;
    ...
    duplicateCanvas(nodeId) {
        ...
        if (!source || source.type !== NODE_TYPES.SECTION) {
            return null;
        }
    ...
    deleteCanvas(nodeId) {
        ...
        if (!node || node.type !== NODE_TYPES.SECTION) {
            return false;
        }
        ...
        const previousIndex = sections.findIndex(
            (section) => section.id === node.id,
        );

        this.state.deleteNode(node.id);

        const remaining = [...this.state.document.sections]
    ...
    #renderList() {
        ...
        for (const [index, section] of sections.entries()) {
            list.append(
                this.#renderItem(section, index, sections.length),
            );
        }
    ...
    #scrollCanvasIntoView(nodeId) {
        const element = this.renderer.root?.querySelector(
            `[data-r3-section-id="${cssEscape(nodeId)}"]`,
        );
    ...
    #syncSelection() {
        const selectedSectionId = this.state.selection.sectionId;

        this.root.querySelectorAll("[data-r3-canvas-id]")
            .forEach((item) => {
                item.classList.toggle(
                    "is-selected",
                    item.dataset.r3CanvasId === selectedSectionId,
                );
            });
    }
