...
export class BuilderState {
    ...
    createNode(type, overrides = {}) {
        return this.transaction("node:create", () => {
            ...
            if (node.type === NODE_TYPES.SECTION) {
                ...
                node.parentId = null;
                node.order = this.#document.sections.length;
    ...
    updateNode(nodeId, patch, options = {}) {
        return this.transaction("node:update", () => {
            ...
            if (next.type !== NODE_TYPES.SECTION) {
    ...
    moveNode(nodeId, newParentId, index = null) {
        return this.transaction("node:move", () => {
            ...
            if (node.type === NODE_TYPES.SECTION) {
                throw new Error(
                    "Una SECTION no puede tener parent."
                );
            }
    ...
    reorderNode(nodeId, index) {
        return this.transaction("node:reorder", () => {
            ...
            if (node.type === NODE_TYPES.SECTION) {
                const list = this.#document.sections;
    ...
    duplicateNode(nodeId, options = {}) {
        return this.transaction("node:duplicate", () => {
            ...
            const clones = subtree.map((node) => {
                ...
                if (
                    source.type === NODE_TYPES.SECTION
                ) {
                    clone.sectionId = idMap.get(
                        node.sectionId
                    );
                } else {
                    clone.sectionId = source.sectionId;
                }
            ...
            if (
                rootClone.type === NODE_TYPES.SECTION
            ) {
                rootClone.parentId = null;
                rootClone.sectionId = rootClone.id;
                this.#document.sections.push(rootClone);
    ...
    #replaceNode(nextNode) {
        const sectionIndex =
            this.#document.sections.findIndex(
                (node) => node.id === nextNode.id
            );

        if (sectionIndex >= 0) {
            this.#document.sections[sectionIndex] = nextNode;
            return;
        }
    ...
    #validateParentAssignment(node) {
        ...
        node.sectionId =
            parent.type === NODE_TYPES.SECTION
                ? parent.id
                : parent.sectionId;

        if (!node.sectionId) {
            throw new Error(
                "No se pudo resolver sectionId."
            );
        }
    }
    ...
    #reindexSections() {
        this.#document.sections.forEach(
            (section, index) => {
                section.order = index;
            }
        );
    ...
    #propagateSectionId(nodeId, sectionId) {
        const node = this.#findNode(nodeId);
