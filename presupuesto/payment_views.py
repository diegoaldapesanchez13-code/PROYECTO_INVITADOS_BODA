...
def revisar_pago_cliente(request, pago_id):
    ...
    if estado not in {'RECIBIDO', 'OBSERVADO'}:
        ...
        return redirect(f"{reverse('dashboard')}?evento={pago.evento_id}#finanzas")

    pago.estado = estado
    pago.revisado_por = request.user
    pago.comentario_equipo = (request.POST.get('comentario_equipo') or '').strip() or None
    pago.fecha_revision = timezone.now()
    pago.save(update_fields=['estado', 'revisado_por', 'comentario_equipo', 'fecha_revision', 'fecha_actualizacion'])
