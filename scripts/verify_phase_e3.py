...
source=VIEWS.read_text(encoding="utf8")
for name in FORBIDDEN:
    if re.search(rf"^def\s+{re.escape(name)}\b",source,re.M):
        raise SystemExit(f"Legacy Python presente: {name}")
